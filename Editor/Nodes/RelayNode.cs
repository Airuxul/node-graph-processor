...
namespace GraphProcessor
{
    ...
    {
        public override Type RuntimeNodeType => typeof(RuntimeRelayNode);
        ...
        [NonSerialized] int outputIndex;

        SerializableType inputType = new(typeof(object));

        const int k_MaxPortSize = 14;

        // SO node only describes ports. Execution logic is in RuntimeRelayNode.
        ...
        public void PushOutputs(List<SerializableEdge> edges, NodePort outputPort)
        {
                ...
                (unpackOutput || inputPortEdges.Count == 1))
            {
                ...
                {
                    edge.passThroughBuffer =
                        edge.inputNode is RelayNode inputRelay && !inputRelay.packInput ? output : data;
                }
            }
            else
            {
        ...
        IEnumerable<PortData> OutputPortBehavior(List<SerializableEdge> edges)
        {
            ...
            if (unpackOutput && inputPortEdges.Count == 1)
            {
                yield return new PortData
                {
                    ...
                    acceptMultipleEdges = true,
                    sizeInPixel =
                        Mathf.Min(k_MaxPortSize, Mathf.Max(underlyingPortData.Count, 1) + 7), // TODO: function
                };
        ...
        static List<(Type, string)> s_empty = new();
        ...
        {
            return GetExportJsonData(new RelayNodeParamData
            {
                PackInput = packInput,
